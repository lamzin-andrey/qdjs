...
Tab.prototype.setPath = function(path) {
	...
	if (this.isSpecialTab()) {
		...
		//this.listRenderer.stop();
		if (this.tabItem && this.tabItem.type == TabPanelItem.TYPE_SEARCH) {
			if (app.searchTabs[app.tabPanel.activeIndex].searchText) {
				app.searchTabs[app.tabPanel.activeIndex].setPath(path);
			}
		}
		return;
	}
...
Tab.prototype.onClickOpen = function() {
	if (app.searchTabs && app.searchTabs[app.tabPanel.activeIndex]) {
		app.searchTabs[app.tabPanel.activeIndex].openAction(window.currentCmTargetId);
	}
	this.openAction(window.currentCmTargetId);
}

Tab.prototype.onClickOpenContainingFolder = function() {
	if (app.searchTabs && app.searchTabs[app.tabPanel.activeIndex]) {
		app.searchTabs[app.tabPanel.activeIndex].onClickOpenContainingFolder(window.currentCmTargetId);
	}
}
...
Tab.prototype.tpl = function() {
	return '<div class="tabContentItem {active}" title="{name} {catalog}">\
...
Tab.prototype.onKeyDown = function(evt) {
	...
	if (this.isSpecialTab()) {
		if(app.searchTabs[app.tabPanel.activeIndex]) {
			app.searchTabs[app.tabPanel.activeIndex].onKeyDown(evt);
			return;
		}
	}
...
Tab.prototype.onScrollDown = function() {
	...
	if (this.isSpecialTab()) {
		if(app.searchTabs[app.tabPanel.activeIndex]) {
			app.searchTabs[app.tabPanel.activeIndex].onScrollDown();
		}
		return;
	}
...
Tab.prototype.onScrollUp = function() {
	...
	if (this.isSpecialTab()) {
		if(app.searchTabs[app.tabPanel.activeIndex]) {
			app.searchTabs[app.tabPanel.activeIndex].onScrollUp();
		}
		return;
	}
...
Tab.prototype.getNextId = function(id) {
	id = this.toI(id);
	id++;
//	alert('sz(this.list) = '  + sz(this.list) + ', id = ' + id);
	if (id < sz(this.list)) {
		return ('f' + id);
	}
