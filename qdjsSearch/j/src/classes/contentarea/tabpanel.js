...
TabPanel.prototype.onClickButton = function(evt){
	...
	currentSearchTab = app.searchTabs[app.tabPanel.activeIndex];
	if (currentSearchTab && currentSearchTab.list) {
		// alert("Save in " + this.activeIndex);
		this.tabs[this.activeIndex].listCopy = mclone(currentSearchTab.list);
	}
	...
	if (currentSearchTab) {
		// alert("Restore " + idx);
		currentSearchTab.skipRequestList = this.tabs[idx].listCopy;
	}
...
TabPanel.prototype.onClickCloseButton = function(evt){
	...
	delete app.searchTabs[this.activeIndex];
