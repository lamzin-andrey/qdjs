...
var Multihost = {
	...
	createCommand : function(file) {
		...
		//copy memcache - it no support now (2019 year)
		//cmd += "\ncp -f " + this.dataDir + '/exts/memcache.so /opt/lampp/lib/php/extensions/no-debug-non-zts-' + ZEND_EXT_DATE + '/memcache.so';
		cmd += "\ncp -f " + this.dataDir + '/exts/memcached.so /opt/lampp/lib/php/extensions/no-debug-non-zts-' + ZEND_EXT_DATE + '/memcached.so';
		cmd += "\ncp -f " + this.dataDir + '/exts/libmemcached.so.11 /opt/lampp/lib/libmemcached.so.11';
		
		//copy xdebug
		cmd += "\ncp -f " + this.dataDir + '/exts/xdebug.so /opt/lampp/lib/php/extensions/no-debug-non-zts-' + ZEND_EXT_DATE + '/xdebug.so';
		
		//copy libimagick
		cmd += "\ncp -f " + this.dataDir + '/exts/imagick.so /opt/lampp/lib/php/extensions/no-debug-non-zts-' + ZEND_EXT_DATE + '/imagick.so';
